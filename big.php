...
<body>
	...
	<?php
		...
		$cluster = mysqli_query($con,"select cluster_big from users where email = '$mail';");
		$cluster_num = mysqli_fetch_row($cluster);
		$clusterNumber = $cluster_num[0];
		if($clusterNumber == 0) {
			$res = mysqli_query($con,"select pid, title, projectPath, views from project where projectId = 3;");
			...
			echo '</h3>';
			//echo $clusterNumber;
			$res = mysqli_query($con,"select pid, title, projectPath, views from project where projectId = 3 and cluster = '$clusterNumber';");
